...
void ECU3_APP_Valid_Access(void);
void ECU3_APP_InValid_Acess(void);
...
static void APP_UART_init(void)
{
	...
	uartCfg.IRQ_CallBack = ECU3_UART_IRQ_CallBack;
...
static void APP_SPI_init(void)
{
	...
	spi1Cfg.P_ISR_CallBack = ECU3_SPI_IRQ_CallBack;
...
void ECU3_APP_Valid_Access(void)
{
	...
	HAL_LCD_Send_String("GoodBye Sir     - Opening Gate");
...
static void ECU3_SPI_IRQ_CallBack(struct S_IRQ_SRC irq_src)
{
	...
	if(irq_src.RXNE)
	{
		...
		if(Slave_TxData == 'T')
			ECU3_APP_Valid_Access();
		else if(Slave_TxData == 'F')
			ECU3_APP_InValid_Acess();
